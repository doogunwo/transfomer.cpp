...
// ==========================================================================
// GTF Kernel: 4-Plane Bit-Serial Dot Product
// "곱셈 없이, 비트를 찢어서 더한다."
// ==========================================================================
void ggml_vec_dot_gtf_q8_0(const int n, float * GGML_RESTRICT s, size_t bs, const void * restrict vx, size_t bx, const void * restrict vy, size_t by, int nrc) {
    // n: 데이터 개수
    // vx: GTF 가중치 (4-Plane)
    // vy: 사용자 입력 (Q8_0, 8비트 정수)
    GGML_UNUSED(bs);
    GGML_UNUSED(bx);
    GGML_UNUSED(by);
    GGML_UNUSED(nrc);
    
    const int nb = n / QK_GTF; // 블록 개수 (256개 단위)
    assert(n % QK_GTF == 0);   // 개수가 딱 떨어져야 함

    const block_gtf * GGML_RESTRICT x = (const block_gtf *)vx;
    const block_q8_0 * GGML_RESTRICT y = (const block_q8_0 *)vy;

    float sumf = 0.0; // 최종 결과 누적
    printf("DEBUG: n=%d, nb=%d (QK_GTF=%d)\n", n, nb, QK_GTF);
    // 블록 단위 루프 (Block Loop)
    for (int i = 0; i < nb; i++) {
        // 1. 스케일링 팩터 미리 계산 (가중치 스케일 * 입력 스케일)
        const float d = x[i].d * GGML_FP16_TO_FP32(y[i].d);
        int sum_int = 0; // 정수부 합계

        if (i == 0) {
             printf("DEBUG: Loop Start. d=%f\n", d);
             printf("DEBUG: LSB[0]=%d, Input[0]=%d\n", x[i].lsb[0], y[i].qs[0]);
        }

        // 2. 비트 시리얼 연산 (256개 요소를 32바이트씩 훑음)
        // AVX2는 나중에 여기에 _mm256_load 등을 넣으면 됨. 지금은 스칼라(기본) 버전.
        for (int j = 0; j < 32; j++) {
            
            // 4개의 비트 평면(Plane) 로드
            uint8_t b0 = x[i].lsb[j]; // 가중치 1
            uint8_t b1 = x[i].b1[j];  // 가중치 2
            uint8_t b2 = x[i].b2[j];  // 가중치 4
            uint8_t b3 = x[i].msb[j]; // 가중치 8 (MSB)

            // 입력 데이터 8개 로드
            const int8_t * qs = &y[i].qs[j * 8];

            // 8비트 하나하나 뜯어보기 (Popcount 논리)
            for (int bit = 0; bit < 8; bit++) {
                int8_t val = qs[bit]; // 입력값 (예: 12)

                if (j == 0 && bit == 0) {
                     printf("DEBUG: Checking bit 0. b0=%d, val=%d. Condition: %s\n", 
                            b0, val, (b0 & (1 << bit)) ? "TRUE" : "FALSE");
                }

                // ★ 여기가 핵심! 곱셈 없이 조건부 덧셈 (Bitwise-AND Accumulate)
                // 비트가 1이면 더하고, 0이면 무시한다.
                
                if (b0 & (1 << bit)) sum_int += val;        // Weight: 1
                if (b1 & (1 << bit)) sum_int += (val << 1); // Weight: 2
                if (b2 & (1 << bit)) sum_int += (val << 2); // Weight: 4
                if (b3 & (1 << bit)) sum_int += (val << 3); // Weight: 8
            }
        }
