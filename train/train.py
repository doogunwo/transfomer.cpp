...
# -----------------------------
# 0) Device: DirectML 우선
# -----------------------------
def get_device():
    try:
        import torch_directml  # type: ignore
        d = torch_directml.device()
        print("[Device] Using DirectML:", d)
        return d
    except Exception as e:
        print("[Device] torch_directml not available:", e)

    if torch.cuda.is_available():
        d = torch.device("cuda")
        print("[Device] Using CUDA:", d)
        return d

    d = torch.device("cpu")
    print("[Device] Using CPU:", d)
    return d
...
def make_causal_mask(T: int, device):
    # DirectML/Windows에서 안전하게: CPU에서 만든 뒤 device로 이동
...
BATCH_SIZE = 8  # iGPU면 16은 부담일 수 있어. 먼저 8 추천
